...
class PathValidator {
  static validatePath(inputPath, workspacePath) {
    ...
    const resolvedWorkspace = path.resolve(workspacePath || process.cwd());
    
    // Check if the resolved path is within the workspace
    if (!resolvedPath.startsWith(resolvedWorkspace + path.sep) && resolvedPath !== resolvedWorkspace) {
      throw new Error(`Security error: Path traversal detected. Path '${inputPath}' is outside workspace '${workspacePath}'`);
    }
    
    return resolvedPath;
  }
  
  static validateUri(uri, workspacePath) {
    ...
    return this.validatePath(filePath, workspacePath);
  }
  
  static safeReadFile(filePath, workspacePath) {
    const validatedPath = this.validatePath(filePath, workspacePath);
  ...
  static safeWriteFile(filePath, content, workspacePath) {
    const validatedPath = this.validatePath(filePath, workspacePath);
    ...
    return fs.writeFileSync(validatedPath, content, 'utf8');
  }
}
...
class LSPDaemon {
  ...
  // Convert URI to file path (with validation)
  uriToFilePath(uri) {
    return PathValidator.validateUri(uri, this.workspacePath);
  }
  ...
  async syncFile(filePath, changeType = 'changed') {
    try {
      ...
      const validatedPath = PathValidator.validatePath(filePath, this.workspacePath);
      
      const uri = this.filePathToUri(validatedPath);
      const relativePath = path.relative(this.workspacePath, validatedPath);
      ...
      const content = PathValidator.safeReadFile(validatedPath, this.workspacePath);
  ...
  async applyTextEdits(filePath, textEdits) {
    ...
    // Security: Validate file path to prevent path traversal
    const validatedPath = PathValidator.validatePath(filePath, this.workspacePath);
    let content = PathValidator.safeReadFile(validatedPath, this.workspacePath);
    ...
    PathValidator.safeWriteFile(validatedPath, content, this.workspacePath);
  ...
  async refactorMoveFile(sourceUri, targetUri) {
    ...
    try {
      ...
      // 4. Move the physical file - Security: Use validated paths
      const validatedSourcePath = PathValidator.validatePath(sourcePath, this.workspacePath);
      const validatedTargetPath = PathValidator.validatePath(targetPath, this.workspacePath);
      fs.renameSync(validatedSourcePath, validatedTargetPath);
  ...
  async refactorMoveFunction(sourceFile, targetFile, functionName, startLine, endLine) {
    ...
    try {
      ...
      const validatedSourceFile = PathValidator.validatePath(sourceFile, this.workspacePath);
      const validatedTargetFile = PathValidator.validatePath(targetFile, this.workspacePath);
      
      // 1. Ensure source file is open
      await this.openDocument(validatedSourceFile);
      
      // 2. Read the function code from source file
      const sourceContent = PathValidator.safeReadFile(validatedSourceFile, this.workspacePath);
      ...
      if (fs.existsSync(validatedTargetFile)) {
        targetContent = PathValidator.safeReadFile(validatedTargetFile, this.workspacePath);
      } else {
      ...
      PathValidator.safeWriteFile(validatedTargetFile, targetContent, this.workspacePath);
      
      // 6. Open target file in LSP
      await this.openDocument(validatedTargetFile);
      ...
      const references = await this.sendRequest('textDocument/references', {
        textDocument: { uri: `file://${validatedSourceFile}` },
      ...
      PathValidator.safeWriteFile(validatedSourceFile, newSourceContent, this.workspacePath);
    ...
    try {
      if (edit.changes) {
        ...
        for (const [uri, changes] of Object.entries(edit.changes)) {
          // Security: Validate file path to prevent path traversal
          const filePath = PathValidator.validateUri(uri, this.workspacePath);
          console.log(`📝 [EDIT] Applying ${changes.length} changes to ${filePath}`);
          
          // Read the current file content
          let content = PathValidator.safeReadFile(filePath, this.workspacePath);
          ...
          PathValidator.safeWriteFile(filePath, content, this.workspacePath);
          console.log(`💾 [SAVED] Updated ${filePath}`);
        }
      }
      ...
      if (edit.documentChanges) {
        ...
        for (const change of edit.documentChanges) {
          if (change.kind === 'create') {
            // Security: Validate file path to prevent path traversal
            const createPath = PathValidator.validateUri(change.uri, this.workspacePath);
            PathValidator.safeWriteFile(createPath, change.options?.contents || '', this.workspacePath);
            console.log(`📄 [CREATE] Created file ${change.uri}`);
          } else if (change.kind === 'rename') {
            // Security: Validate both old and new file paths
            const oldPath = PathValidator.validateUri(change.oldUri, this.workspacePath);
            const newPath = PathValidator.validateUri(change.newUri, this.workspacePath);
            fs.renameSync(oldPath, newPath);
            console.log(`🔄 [RENAME] Renamed ${change.oldUri} to ${change.newUri}`);
          } else if (change.kind === 'delete') {
            // Security: Validate file path to prevent path traversal
            const deletePath = PathValidator.validateUri(change.uri, this.workspacePath);
      ...
      req.on('end', async () => {
        try {
          ...
          if (lspRequest.params?.textDocument?.uri) {
            try {
              // Security: Validate URI to prevent path traversal
              const filePath = PathValidator.validateUri(lspRequest.params.textDocument.uri, daemon.workspacePath);
              const synced = await daemon.ensureFileSync(filePath);
              if (!synced) {
                console.log(`⚠️  [SKIP] Unsupported file type: ${filePath}`);
              }
            } catch (error) {
              console.error(`🚨 [SECURITY] Invalid file path in LSP request: ${error.message}`);
              res.writeHead(400, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ error: `Invalid file path: ${error.message}` }));
              return;
            }
          }
      ...
      req.on('end', async () => {
        try {
          ...
          if (filePath) {
            try {
              // Security: Validate file path to prevent path traversal
              const validatedPath = PathValidator.validatePath(filePath, daemon.workspacePath);
              const synced = await daemon.ensureFileSync(validatedPath);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              ...
              console.error(`🚨 [SECURITY] Invalid file path in sync request: ${error.message}`);
              res.writeHead(400, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ error: `Invalid file path: ${error.message}` }));
              return;
            }
...
// Export function to start daemon with configuration
function startDaemon(config = {}) {
  const daemon = new LSPDaemon(config.WORKSPACE_PATH, config.TSCONFIG_PATH);
  
  // Override default port
  const port = config.PORT || process.env.LSP_DAEMON_PORT || 3007;
  
  return startServer(daemon, port);
}
